...
class BeautifulSoupParser(BaseParser):
    ...
    def parse_video_card(self, card_html: str) -> ParsedVideo:
        try:
            ...
            # 1) Container by id
            container = soup.find(
                "div", attrs={"id": re.compile(r"^waterfall_item_\d+$")}
            )
            ...
            logger.info(f"Attribute error: {e}. Saving HTML")
            with open("parser_error.html", "w") as f:
                f.write(card_html)
            raise e
